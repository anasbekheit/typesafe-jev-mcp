...
pub struct Input {
    ...
    #[serde(default)]
    #[schemars(
        description = "model to use; defaults to jev-latest. jev-preview is an early build of the same model. Pass an exact versioned id such as jev-1.13.0 to pin a tuned threshold"
    )]
    model: Option<String>,
}
